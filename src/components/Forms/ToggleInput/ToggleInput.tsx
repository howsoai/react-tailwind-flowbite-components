...
import { Label, LabelProps, type FlowbiteColors } from "flowbite-react";
import { forwardRef } from "react";
import { twMerge } from "tailwind-merge";

const theme: ToggleInputTheme = {
  root: {
  ...
  (
    ...
    return (
      <Label
        role="switch"
        {...labelProps}
        className={twMerge(theme.root.base, className, labelProps?.className)}
      >
        <input type="checkbox" className="peer sr-only" ref={ref} {...props} />
        <div
          className={twMerge(
            theme.toggle.base,
            theme.toggle.size[size],
            theme.toggle.checked.off,
            theme.toggle.checked.on,
